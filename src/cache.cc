...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if (cache_type == IS_LLC) {
          ...
          uint32_t least_set = finder.findLeastUsedSet();
        ...
        if (do_fill){
          if(cache_type == IS_LLC)
          {
           ...
           // least_set=set;
            way = llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, least_set, block[least_set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type);
            remap[make_pair(set, MSHR.entry[mshr_index].address)] = make_pair(least_set, way);
            set = least_set;
          }
            // update prefetcher
...
void CACHE::handle_writeback()
{
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        if(cache_type==IS_LLC)
        {
           if(remap.find({set, RQ.entry[index].address}) != remap.end())
               {
                 uint32_t l_set = remap[{set, RQ.entry[index].address}].first;
                //  way = remap[{set, RQ.entry[index].address}].second;
                index = l_set;
                 int way = check_hit(&WQ.entry[index]);
                  set = l_set;
               } 
        ...
        if (way >= 0) { // writeback hit (or RFO hit for L1D)
            ...
            if (cache_type == IS_LLC) {
             //   uint32_t least_set = finder.findLeastUsedSet();
                ...
                //  LFUSetFinder finder;
                finder.incrementAccessCount(set); 
            }
            else
